...
python3 create_flashable_firmware.py -L $zip
rm $zip; done
...
for file in *.zip; do
	...
	python telegram.py -t $bottoken -c @XiaomiFirmwareUpdater -M "New stable firmware-less update available!
	*Device*: $model
	*Codename*: $codename
	*Version*: $version
	*Android*: $android
	...
	*MD5*: $md5
	*Download Links*:
	[Osdn](https://osdn.net/projects/xiaomifirmwareupdater/storage/firmware-less/Stable/$bigversion/$codename/)
